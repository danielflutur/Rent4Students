...
function InvoiceViewPaymentAssociation({ isOpen, handleModal, invoice }) {
  const {
    name,
    number,
    amount,
  ...
  // State pentru imaginea de previzualizare și controlul vizibilității modalului de preview
  const [previewImage, setPreviewImage] = useState(null);
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);

  // Handler pentru click pe imagine: deschide modalul de preview
  const handleImageClick = (imageUrl) => {
    setPreviewImage(imageUrl);
    setIsPreviewOpen(true);
  };

  // Handler pentru închiderea modalului de preview
  const closePreview = () => {
  ...
  return (
    <>
                ...
                <div className="col-lg-6 col-md-6 col-12">
                  <ul className="homec-modal__list homec-modal__list--v2 list-none">
                    <li>
                      ...
                      <img
                        src={receiptImage}
                        alt="Chitanța de la asociație"
                        className="homec-modal__image"
                        onClick={() => handleImageClick(receiptImage)}
                      />
                    </li>
                    <li>
                      <span>Tabel Asociatie:</span>
                      <img
                        src={tableImage}
                        alt="Tabelul de la asociație"
                        className="homec-modal__image"
                        onClick={() => handleImageClick(tableImage)}
                      />
                    </li>
                  </ul>
                </div>
              ...
              <div className="homec-invoices homec-invoice-table--v2 mg-top-30">
                <table className="homec-invoice-table m-0">
                  <thead className="homec-invoice-table__head">
                    <tr>
                      <th className="homec-invoice-table__column1">An</th>
                      <th className="homec-invoice-table__column2">Lună</th>
                      <th className="homec-invoice-table__column3">Sumă</th>
                      <th className="homec-invoice-table__column4">Status</th>
                      <th className="homec-invoice-table__column5">Dată plată</th>
                      <th className="homec-invoice-table__column6">Factura</th>
                    </tr>
                  </thead>
      ...
      {isPreviewOpen && (
        <ImagePreviewModal previewImage={previewImage} onClose={closePreview} />
      )}
    </>
  );
}

InvoiceViewPaymentAssociation.propTypes = {
  isOpen: PropTypes.bool.isRequired,
  handleModal: PropTypes.func.isRequired,
  invoice: PropTypes.object.isRequired,
};

export default InvoiceViewPaymentAssociation;
